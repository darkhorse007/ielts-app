...
12. `/health` 在保持 `status: "ok"` 健康检查契约不变的前提下，可额外暴露 `reminder_dispatch_scheduler` snapshot；`/internal/reminders/scheduler-status` 与 `/internal/reminders/push-status` 则提供更直接的内部排障视图
