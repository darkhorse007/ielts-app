...
5. 当前仓库已经补上移动端自动化基线：route-level vitest smoke，以及基于 Maestro 的 iOS simulator 与 Android emulator/device 设备级 smoke 脚本、flow 和 CI workflow；其中 account smoke 已覆盖 reminder recommendation、本地提醒安排、远程设备同步与账号删除链路，另有 reminder notification smoke 覆盖通知桥接后的 deep link 恢复。
